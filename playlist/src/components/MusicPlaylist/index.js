...
class MusicPlaylist extends Component {
  ...
  onDelete = id => {
    ...
    const updatePlayList = playlist.filter(eachItem => eachItem.id !== id)
    this.setState({playlist: updatePlayList})
  }

  getSearchResult = () => {
    const {inputSearch, playlist} = this.state
    const searchResult = playlist.filter(eachItem =>
  ...
  render() {
    ...
    const noItem = searchResult.length === 0
    ...
    return (
      <AppContainer>
        ...
        <PlaylistContainer>
          {noItem ? (
            <NoItemFoundContainer>
              <NoItemFound>No Songs Found</NoItemFound>
            </NoItemFoundContainer>
          ) : (
            <Playlist>
              {searchResult.map(eachSongs => (
                <SongItemCard
                  key={eachSongs.id}
                  songItemDetails={eachSongs}
